...
(() => {
  ...
  function refreshAll() {
    ...
    try { renderExportPersonList(); } catch(e) {}
    ...
    if (selPerson()) try { renderExportShiftDetail(selPerson()); } catch(e) {}
  }
  ...
  window.deletePerson = function(name) {
    ...
    renderExportPersonList();
    // Clear export detail if it was showing this person
    const detail = document.getElementById('export-shift-detail');
    if (detail) detail.innerHTML = '<div style="padding:20px;color:var(--text-dim)">← Klikni osobu za pregled</div>';
  ...
  window.deleteShift = function(date,person){
    ...
    renderExportPersonList();
    // Refresh export detail if it was showing this person
    if (document.getElementById('export-shift-detail') && selPerson()===person) {
      if (shifts().some(s=>s.person===person)) renderExportShiftDetail(person);
      else document.getElementById('export-shift-detail').innerHTML = '<div style="padding:20px;color:var(--text-dim)">← Klikni osobu za pregled</div>';
    }
  ...
  function renderExportPersonList() {
    ...
    list.innerHTML = ppl.map(name => {
      ...
      const active = selPerson()===name ? 'active' : '';
  ...
  window.exportSelectPerson = function(name) {
    ...
    document.getElementById('cal-person-filter').value = name;
    renderCalendar();
  };
  ...
  function renderExportShiftDetail(name) {
    ...
    if (!sh.length) { detail.innerHTML='<div style="padding:20px;color:var(--text-dim)">Nema smjena.</div>'; return; }
    ...
    detail.innerHTML = `
      <div class="shift-detail-header">
        ${avatarHTML(name, 42, true)}
        <div>
          <div style="font-size:15px;font-weight:700;color:${pc}">${eh(name)}</div>
      ...
      <div class="shift-detail-list">
        ${sh.map(s => {
          ...
          return `<div class="shift-detail-row">
            ...
            <span class="shift-detail-day" style="color:var(--text-mid)">${DAY_HR[d.getDay()].slice(0,3)}</span>
            <span class="shift-detail-time">${s.startTime}–${s.endTime}</span>
            <span><span style="background:${m.bg};border:1px solid ${m.color};color:${m.text};padding:2px 7px;border-radius:12px;font-size:11px;font-weight:600">${m.icon} ${m.label}</span></span>
  ...
  function openShiftModal(shiftOrNull){
    ...
    if (shiftOrNull) {
      ...
      // Clear person field so user can freely type without pre-selected name blocking
      pInp.value = '';
    ...
    // Delay focus so datalist doesn't steal it on open
    setTimeout(() => { pInp.focus(); pInp.select(); }, 150);
  }
  ...
  function saveShift(){
    ...
    renderExportPersonList();
    renderExportShiftDetail(person);  // immediately show new shift in export
