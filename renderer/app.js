...
(() => {
  ...
  window.uploadAvatar = async function(name) {
    ...
    // Re-render wherever avatars appear
    renderPersonGrid();
  ...
  function setupTitleBar() {
    ...
    document.getElementById('project-switcher').ondblclick = () => {
      const p = proj(); if (!p) return;
      const name = prompt('Preimenuj raspored:', p.name);
  ...
  window.renamePerson = function(oldName) {
    const newName = prompt(`Preimenuj osobu "${oldName}":`, oldName);
